  ...
  describe("WitmonDecoratorBase", async () => {
    ...
    describe("randomUniform(bytes32,uint256,uint8)", async () => {
      ...
      it("variable result when called with same seed and range but different phenotype", async () => {
        const rnd1 = await decorator.randomUniform.call(phenotype1, 0, 254)
        const rnd2 = await decorator.randomUniform.call(phenotype2, 0, 254)
        assert(rnd1.toString() !== rnd2.toString())
      })
      it("variable result when called with same phenotype and range but different seed", async () => {
        const rnd1 = await decorator.randomUniform.call(phenotype1, 0, 128)
        const rnd2 = await decorator.randomUniform.call(phenotype1, 1, 128)
        assert(rnd1.toString() !== rnd2.toString())
      })
      it("same result when called with same parameters", async () => {
        const rnd1 = await decorator.randomUniform.call(phenotype2, 2, 4)
        const rnd2 = await decorator.randomUniform.call(phenotype2, 2, 4)
    ...
    describe("randomUniformBase2(bytes32,uint256,uint8)", async () => {
      ...
      it("variable result when called with same seed and range but different phenotype", async () => {
        const rnd1 = await decorator.randomUniformBase2.call(
          phenotype1,
          0,
          8
        )
        const rnd2 = await decorator.randomUniformBase2.call(
          phenotype2,
          0,
          8
        )
